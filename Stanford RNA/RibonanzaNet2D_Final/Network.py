    ...
    def forward(self, src, pairwise_features, src_mask=None, return_aw=False, use_gradient_checkpoint=False):
        ...
        if use_gradient_checkpoint:
            pairwise_features = pairwise_features + \
                checkpoint.checkpoint(self.custom(
                    self.outer_product_mean), src)
            ...
            pairwise_features = pairwise_features + \
                self.pair_dropout_out(self.triangle_update_out(
                    pairwise_features, src_mask))
            pairwise_features = pairwise_features + \
                self.pair_dropout_in(self.triangle_update_in(
                    pairwise_features, src_mask))
        if self.use_triangular_attention:
            pairwise_features = pairwise_features + \
                self.pair_attention_dropout_out(
                    self.triangle_attention_out(pairwise_features, src_mask))
            pairwise_features = pairwise_features + \
                self.pair_attention_dropout_in(
    ...
    def forward(self, src, src_mask=None, return_aw=False):
        ...
        for i, layer in enumerate(self.transformer_encoder):
            ...
            src, pairwise_features = layer(
                src, pairwise_features, src_mask, return_aw=return_aw, use_gradient_checkpoint=self.use_gradient_checkpoint)
...
class TriangleAttention(nn.Module):
    ...
    def forward(self, z, src_mask):
        ...
        q, k, v = map(lambda x: rearrange(
            x, 'b i j (h d)->b i j h d', h=self.n_heads), (q, k, v))
